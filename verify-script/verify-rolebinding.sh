...
function verify() {
	...
	readarray -t obj_res < <(oc --kubeconfig $kube_config_source get $obj_target -n $project_name --no-headers | egrep -v "system:deployers|system:image-builders|system:image-pullers" | awk '{print $1}')
