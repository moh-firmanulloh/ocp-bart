...
#
#
obj_target="configmap"
GREEN='\033[0;32m'
NC='\033[0m'
exclude_pattern="^openshift|^kube"
...
if [[ "$#" -eq 9 ]];then
	...
	echo "$0 projectName ocpSourceApiURL ocpSourceUser ocpSourcePass ocpTargetApiURL ocpTargetUser ocpTargetPass configMapMigrate baseDir"
	exit 0
...
if [[ "$#" -ne 9 && -z "$config_file" ]];then
	echo "Define \$config_file or"
        echo "Usage: $0 projectName ocpSourceApiURL ocpSourceUser ocpSourcePass ocpTargetApiURL ocpTargetUser ocpTargetPass configMapMigrate baseDir"
        exit 1
fi
...
function verify() {
	...
	if [[ "${#obj_res[@]}" -ne 0 ]];then
		for resource in "${obj_res[@]}";do
			...
			source_hash=$(oc --kubeconfig $kube_config_source get $obj_target $resource -n $project_name -ojson | jq -r 'del(.metadata.creationTimestamp,.metadata.resourceVersion, .metadata.uid, .metadata.ownerReferences, .status)' | sha256sum)
			target_hash=$(oc --kubeconfig $kube_config_target get $obj_target $resource -n $project_name -ojson | jq -r 'del(.metadata.creationTimestamp,.metadata.resourceVersion, .metadata.uid, .metadata.ownerReferences, .status)' | sha256sum)
