...
if [[ "$#" -eq 9 ]];then
	...
	ocp_target_user=$6
	ocp_target_pass=$7
	deploymentconfig_param=$8
	base_dir="${9%/}"
elif [[ $BUILDING_ARGS ]];then
	echo "$0 projectName ocpSourceApiURL ocpSourceUser ocpSourcePass ocpTargetApiURL ocpTargetUser ocpTargetPass deploymentConfigMigrate baseDir"
	exit 0
elif [[ ! -z "$config_file" && -f "$config_file" ]];then
        source "$config_file"
else
        echo "Define \$config_file or"
	echo "Usage: $0 projectName ocpSourceApiURL ocpSourceUser ocpSourcePass ocpTargetApiURL ocpTargetUser ocpTargetPass deploymentConfigMigrate baseDir"
	exit 1
fi
...
function verify() {
	...
	if [[ "${#obj_res[@]}" -ne 0 ]];then
		for resource in "${obj_res[@]}";do
			...
			source_hash=$(oc --kubeconfig $kube_config_source get $obj_target $resource -n $project_name -ojson 2>/dev/null | jq -r 'del(.metadata, .status)' | sha256sum | awk '{print $1}')
			target_hash=$(oc --kubeconfig $kube_config_target get $obj_target $resource -n $project_name -ojson 2>/dev/null | jq -r 'del(.metadata, .status)' | sha256sum | awk '{print $1}')
