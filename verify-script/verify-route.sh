...
if [[ "$#" -eq 10 ]];then
	...
	remove_cert=${10}
        ...
        echo "$0 projectName ocpSourceApiURL ocpSourceUser ocpSourcePass ocpTargetApiURL ocpTargetUser ocpTargetPass routeMigrate baseDir removeTLS"
        exit 0
...
if [[ "$#" -ne 10 && -z "$config_file" ]];then
        ...
        echo "Usage: $0 projectName ocpSourceApiURL ocpSourceUser ocpSourcePass ocpTargetApiURL ocpTargetUser ocpTargetPass routeMigrate baseDir removeTLS"
        exit 1
fi
...
function verify() {
	...
	if [[ "${#obj_res[@]}" -ne 0 ]];then
		for resource in "${obj_res[@]}";do
			...
			if [[ $remove_cert ]];then
				source_hash=$(oc --kubeconfig $kube_config_source get $obj_target $resource -n $project_name -ojson | jq -r 'del(.metadata.creationTimestamp,.metadata.resourceVersion, .metadata.uid, .metadata.ownerReferences, .status, .metadata.generation, .spec.tls)' | sha256sum | awk '{print $1}')
				target_hash=$(oc --kubeconfig $kube_config_target get $obj_target $resource -n $project_name -ojson | jq -r 'del(.metadata.creationTimestamp,.metadata.resourceVersion, .metadata.uid, .metadata.ownerReferences, .status, .metadata.generation, .spec.tls)' | sha256sum | awk '{print $1}')
