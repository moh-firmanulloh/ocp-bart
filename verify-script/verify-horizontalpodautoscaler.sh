...
#
#
obj_target="horizontalpodautoscaler"
GREEN='\033[0;32m'
NC='\033[0m'
...
if [[ "$#" -eq 9 ]];then
	...
	ocp_target_user=$6
	ocp_target_pass=$7
	horizontalpodautoscaler_param=$8
	base_dir=${9%/}
elif [[ $BUILDING_ARGS ]];then
        echo "$0 projectName ocpSourceApiURL ocpSourceUser ocpSourcePass ocpTargetApiURL ocpTargetUser ocpTargetPass hpaMigrate baseDir"
        exit 0
...
if [[ "$#" -ne 9 && -z "$config_file" ]];then
        ...
        echo "Usage: $0 projectName ocpSourceApiURL ocpSourceUser ocpSourcePass ocpTargetApiURL ocpTargetUser ocpTargetPass hpaMigrate baseDir"
        exit 1
fi
...
if [[ $horizontalpodautoscaler_param == "true" ]];then
