...
if [[ "$#" -eq 6 ]];then
	...
	echo "$0 projectName ocpTargetApiURL ocpTargetUser ocpTargetPass configMapMigrate baseDir"
	exit 0
...
if [[ "$#" -ne 6 && -z "$config_file" ]];then
	...
	echo "Usage: $0 projectName ocpTargetApiURL ocpTargetUser ocpTargetPass configMapMigrate baseDir"
	exit 1
fi
