...
if [[ "$#" -eq 6 ]];then
	...
	echo "$0 projectName ocpTargetApiURL ocpTargetUser ocpTargetPass secretMigrate baseDir"
	exit 0
...
if [[ "$#" -ne 6 && -z "$config_file" ]];then
	...
	echo "Usage: $0 projectName ocpTargetApiURL ocpTargetUser ocpTargetPass secretMigrate baseDir"
	exit 1
fi
...
function restore() {
	...
	if [[ -d $work_dir ]];then
		...
		for file in $(ls $work_dir);do	
			oc --kubeconfig $kube_config create -f $work_dir/$file -n $project_name
		done
	else
