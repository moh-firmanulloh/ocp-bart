...
if [[ "$#" -eq 6 ]];then
	...
	ocp_user=$3
	ocp_pass=$4
	horizontalpodautoscaler_param=$5
	base_dir=$6
elif [[ $BUILDING_ARGS ]];then
	echo "$0 projectName ocpSourceApiURL ocpSourceUser ocpSourcePass hpaMigrate baseDir"
	exit 0
...
if [[ "$#" -ne 6 && -z "$config_file" ]];then
	...
	echo "Usage: $0 projectName ocpSourceApiURL ocpSourceUser ocpSourcePass hpaMigrate baseDir"
	exit 1
fi
