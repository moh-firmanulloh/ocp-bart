...
if [[ "$#" -eq 7 ]];then
	...
	echo "$0 projectName ocpApiURL ocpUser ocpPass pvcMigrate baseDir newStorageClass"
	exit 0
...
if [[ "$#" -ne 7 && -z "$config_file" ]];then
	...
	echo "Usage: $0 projectName ocpApiURL ocpUser ocpPass pvcMigrate baseDir newStorageClass"
	exit 1
fi
