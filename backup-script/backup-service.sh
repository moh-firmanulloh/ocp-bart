...
if [[ "$#" -eq 6 ]];then
	...
	echo "$0 projectName ocpSourceApiURL ocpSourceUser ocpSourcePass serviceMigrate baseDir"
	exit 0
...
if [[ "$#" -ne 6 && -z "$config_file" ]];then
	...
	echo "Usage: $0 projectName ocpSourceApiURL ocpSourceUser ocpSourcePass serviceMigrate baseDir"
	exit 1
fi
	...
	if [[ "${#obj_res[@]}" -ne 0 ]];then
		...
		for resource in "${obj_res[@]}";do
			echo "Backing up $obj_target: $resource JSON"
	       		oc --kubeconfig $kube_config get $obj_target $resource -n $project_name -ojson | jq -r 'del(.metadata.creationTimestamp,.metadata.resourceVersion, .metadata.uid, .metadata.ownerReferences, .status, .spec.clusterIP, .spec.clusterIPs)' > $work_dir/$obj_target-$resource.json
