...
if [[ "$#" -eq 6 ]];then
	...
	ocp_user=$3
	ocp_pass=$4
	serviceaccount_param=$5
	base_dir=$6
elif [[ $BUILDING_ARGS ]];then
	echo "$0 projectName ocpSourceApiURL ocpSourceUser ocpSourcePass serviceAccountMigrate baseDir"
	exit 0
...
if [[ "$#" -ne 6 && -z "$config_file" ]];then
	...
	echo "Usage: $0 projectName ocpSourceApiURL ocpSourceUser ocpSourcePass serviceAccountMigrate baseDir"
	exit 1
fi
...
function backup() {
	...
	readarray -t obj_res < <(oc --kubeconfig $kube_config get $obj_target -n $project_name --no-headers | egrep -wv "builder|default|deployer" | awk '{print $1}')
	if [[ "${#obj_res[@]}" -ne 0 ]];then
		mkdir -p $(echo $work_dir)
		for resource in "${obj_res[@]}";do
			echo "Backing up $obj_target: $resource JSON"
	       		oc --kubeconfig $kube_config get $obj_target $resource -n $project_name -ojson | jq -r '.secrets |= map(select(.name | test("-dockercfg-") | not)) | .imagePullSecrets |= map(select(.name | test("-dockercfg-") | not)) | del(.metadata.creationTimestamp,.metadata.resourceVersion, .metadata.uid, .status)' > $work_dir/$obj_target-$resource.json
